...
Table~\ref{tab:summary} summarizes the performance of all models on both datasets. On the synthetic dataset, we observe consistent improvement as memory components are added: Model 0 (Base) achieves 0.325 STM and 0.700 LTM accuracy at epoch 9. Model 1 improves STM to 0.425. Model 2 achieves 0.375 STM and 0.750 LTM. Model 3 achieves 0.375 STM and 0.800 LTM at epoch 5---notably reaching peak performance earlier. Model 4 achieves the highest STM accuracy of 0.550, though LTM decreases to 0.750.

On the real dataset (Dog-Cat), Model 4 shows 0.000 accuracy across all epochs, indicating the model struggles with this domain. However, loss decreases from 2.5025 to 1.4264, and difflib scores increase, suggesting gradual learning.

\begin{table}[h]
\centering
\caption{Performance comparison on synthetic and real datasets (best epoch).}
\label{tab:summary}
\begin{tabularx}{\textwidth}{@{}lXXXX@{}}
\toprule
Model & Syn. Epoch & Syn. STM/LTM & Real STM Acc & Real LTM Acc \\
\midrule
0 (Base) & 9 & 0.325 / 0.700 & 0.000 & 0.000 \\
1 (SumOnly) & 9 & 0.425 / 0.700 & 0.000 & 0.000 \\
2 (SumTokLimit) & 9 & 0.375 / 0.750 & 0.000 & 0.000 \\
3 (SumTokNer) & 5 & 0.375 / 0.800 & 0.000 & 0.000 \\
4 (FullMemory) & 9 & 0.550 / 0.750 & 0.000 & 0.000 \\
\bottomrule
\end{tabularx}
\end{table}
...
Table~\ref{tab:real} shows the real dataset results, demonstrating loss reduction and difflib score improvement for Model 4 despite 0.000 accuracy.
...
\begin{itemize}
    \item \textbf{Training}: $\sim$10 epochs per model, with each epoch taking approximately [TO BE FILLED] minutes
    \item \textbf{Pre-computation}: Summarization and NER features are cached to avoid recomputation
    \item \textbf{Total Training Time}: [TO BE FILLED] hours for all 5 models on both datasets
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Dependencies}: PyTorch, OpenAI API (for LLM-as-a-judge), transformers (for summarization/NER)
\end{itemize}
